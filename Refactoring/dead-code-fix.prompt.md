...
The audit writes to `<root>/dead-code-<timestamp>.md`. Resolve
`<root>` in this order: `.playbook-audits/` if it exists, else
`docs/audits/` if that exists (legacy convention). Look for files
matching `<root>/dead-code-*.md` and pick the most recent
(`ls -1 <root>/dead-code-*.md 2>/dev/null | sort | tail -1` — the
