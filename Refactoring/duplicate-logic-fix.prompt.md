...
The audit writes to `docs/audits/duplicate-logic-<timestamp>.md` when
the project has a `docs/` directory, and inline otherwise. Look for
files matching `docs/audits/duplicate-logic-*.md` and pick the most
recent (`ls -1 docs/audits/duplicate-logic-*.md 2>/dev/null | sort | tail -1`
— the `YYYYMMDDTHHMMSS` suffix sorts lexicographically). If no
report exists there, ask the user whether they have an inline report
to paste, or whether they need to run the audit.
