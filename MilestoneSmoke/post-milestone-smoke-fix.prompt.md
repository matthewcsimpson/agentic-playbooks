...
Read the most recent smoke-test report. Resolve `<root>` in this
order: `.playbook-audits/` if it exists, else `docs/` if
`docs/smoke-tests/` exists (legacy convention). Files are named
`<root>/smoke-tests/<tag>-<timestamp>.md` (e.g.
`.playbook-audits/smoke-tests/v1.6.0-20260519T143022.md`); pick
the latest by sorting lexicographically — both `<tag>` and the
`YYYYMMDDTHHMMSS` suffix sort correctly
(`ls -1 <root>/smoke-tests/*.md 2>/dev/null | sort | tail -1`).
If the user named a specific tag, restrict to that tag first
(`ls -1 <root>/smoke-tests/<tag>-*.md | sort | tail -1`). If
neither root exists or any report can't be found, surface that
and stop — there's nothing to action.
...
under `<root>/smoke-tests/<tag>-<timestamp>/`) to judge whether two failures
share a cause.
