...
# Top-of-file marker for the project-local generated catalogs (AGENTS.md and
# .github/copilot-instructions.md). Lets the agent-instructions audit / init
# playbooks recognise these as generated and never pick them as a hand-authored
# canonical (their next regen would clobber any rules moved in). The first line
# is the exact strict phrase those heuristics match; the second carries detail.
# It sits in the file's top region (an HTML comment block before any other
# content), which is where the heuristics look.
GENERATED_MARKER = (
    "<!-- AUTO-GENERATED -->\n"
    "<!-- Generated by tools/generate-adapters.py from prompt frontmatter — "
    "DO NOT EDIT. Edit the prompts and re-run the generator. -->\n"
    "\n"
)
...
def emit_agents_md(out_file: Path, prompts: list[Prompt], *, absolute_root: Path | None = None) -> None:
    # Project-local AGENTS.md carries the generated marker so the
    # agent-instructions audit / init playbooks treat it as generated. (The
    # Codex global AGENTS.md goes through write_codex_agents, which merges into
    # a possibly hand-written file, so it is intentionally not marked here.)
    write(out_file, GENERATED_MARKER + render_agents_body(prompts, absolute_root=absolute_root))
    ...
    for collection, items in group_by_collection(prompts).items():
        ...
        for p in items:
            ...
            if p.related:
                ...
                lines.append(f"  Related: {related}")
        lines.append("")
    write(out_file, GENERATED_MARKER + "\n".join(lines))
