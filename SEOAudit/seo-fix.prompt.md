...
The audit writes to `<root>/audits/seo-<timestamp>.md`. Resolve
`<root>` in this order: `.playbook-audits/` if it exists, else
`docs/` if `docs/audits/` exists (legacy convention). Look for
files matching `<root>/audits/seo-*.md` and pick the most recent
(`ls -1 <root>/audits/seo-*.md 2>/dev/null | sort | tail -1` —
