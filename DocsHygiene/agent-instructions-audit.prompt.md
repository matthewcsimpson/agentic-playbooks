...
pointless — the next regeneration overwrites it. Read the first ~20
lines of each candidate; if any line matches a generation marker, skip
the file and note it in the report's "Files audited" line as
`<file> (skipped — auto-generated)`. Markers to detect:
