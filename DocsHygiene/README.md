...
| Prompt | What it does |
|---|---|
| `agent-instructions-init.prompt.md` | Sets up a project's agent / LLM instructions at the **start** of a project by interviewing the user for their stack and convention preferences — naming, layout / test-file location, imports, error handling, commit conventions, and more — with opinionated, stack-aware recommended defaults and an accept-all fast path. Writes a well-formed canonical `AGENTS.md` and points each tool the project uses (`CLAUDE.md`, Cursor rules, Copilot instructions) at it, referencing whatever enforces each rule. Distinct from the audit, which observes existing code: init *sets* the rules, the audit *checks* them. Merges into a hand-authored file rather than clobbering. Leaves files for review; does not commit. |
...
sets the rules at project start (interviewing for preferences),
`agent-instructions-audit` grades them against the code as the project
ages, and `agent-instructions-fix` actions the audit's findings.
