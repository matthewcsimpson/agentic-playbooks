...
| Prompt | What it does |
|---|---|
| `agent-instructions-init.prompt.md` | Generates a well-formed agent / LLM instructions file from scratch, derived from the actual codebase and recent PRs (not boilerplate). Establishes evidence-backed code-convention rules (naming, imports, layout, constants, error handling, …) grouped by topic, each concrete with examples / exceptions and a pointer to whatever already enforces it. Writes a canonical `AGENTS.md` (default) and points each tool the project uses (`CLAUDE.md`, Cursor rules, Copilot instructions) at it, so there's a single source of truth. If a hand-authored file already exists, switches to merge/augment mode instead of clobbering. Leaves files for review; does not commit. |
