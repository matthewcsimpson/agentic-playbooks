...
- The target file is **not auto-generated**. Read the file's first
  ~20 lines and check for generation markers (`generated by`,
  `DO NOT EDIT`, `@generated`, `<!-- generated -->`,
  `auto-generated`). If any match, skip every finding against that
  file and record "skipped — file is auto-generated; edit the source
  instead." The audit should already have filtered these in its Step
  1, but a defensive re-check here protects against stale audit
  reports and audits that ran before this skip-rule existed.
...
4. **Verify** the mechanism actually fires: run the relevant
   ...
   b. Use a path the project's `.gitignore` already excludes
      ...
      If no such path exists, **add one entry to `.gitignore`
      for the run** and commit the gitignore change separately
      *before* creating the fixture, so the fixture never enters
      tracked state.
   c. After running the verify command and confirming the rule
      fires, **delete the fixture file**, then run
      `git status --porcelain` — output must be empty (the
      fixture is gone and the only staged change is the
      mechanism's config). Do not proceed to the commit if
      `git status` shows anything unexpected.
